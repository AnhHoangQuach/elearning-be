...
const postCourse = async (req, res, next) => {
  try {
    ...
    if (currentPrice && parseInt(currentPrice) < 0) {
      return res
        .status(400)
        .json({ message: "currentPrice phải lớn hơn hoặc bằng 0" });
    }
    if (originalPrice && parseInt(originalPrice) < 0) {
      return res
        .status(400)
        .json({ message: "originalPrice phải lớn hơn hoặc bằng 0" });
    }
    if (
      originalPrice &&
      currentPrice &&
      parseInt(originalPrice) < parseInt(currentPrice)
    ) {
      return res
        .status(400)
        .json({ message: "originalPrice phải lớn hơn hoặc bằng currentPrice" });
    }

    // tính giảm giá
    let saleOff =
      (1 - parseInt(currentPrice) / parseInt(originalPrice)) * 100 || 0;
    // tạo khoá học
...
const putCourse = async (req, res, next) => {
  try {
    ...
    if (
      account.role !== "admin" &&
      JSON.stringify(user._id) !== JSON.stringify(course.author)
    ) {
      return res.status(401).json({ message: "Not permited" });
    }
    ...
    if (
      account.role == "admin" &&
      (newCourse.status == "denied" || newCourse.status == "update denied")
    ) {
      let author = await UserModel.findById({ _id: course.author }).populate(
        "account"
      );
    ...
    if (account.role == "admin" && newCourse.status == "approved") {
      ...
      await LessonModel.updateMany(
        { chapter: { $in: objIdsChapters }, publish: false },
        { publish: true }
      );
    }
    try {
...
const getCourses = async (req, res, next) => {
  try {
    ...
    if (publish) {
      aQuery.splice(1, 0, { $match: { publish: publish == "true" } });
      aCountQuery.splice(1, 0, { $match: { publish: publish == "true" } });
    }
    ...
    if (category && category !== "all") {
      aQuery.push({ $match: { "category.slug": category } });
      aCountQuery.push({ $match: { "category.slug": category } });
    }
    // tìm status
    if (status) {
      aQuery.splice(1, 0, { $match: { status: status } });
      aCountQuery.splice(1, 0, { $match: { status: status } });
    }
    // tìm theo level
    if (level) {
      aQuery.push({ $match: { level: level } });
      aCountQuery.push({ $match: { level: level } });
    }
    // tìm theo giá từ min-max
    if (min) {
      min = parseInt(min);
      aQuery.push({ $match: { currentPrice: { $gte: min } } });
      aCountQuery.push({ $match: { currentPrice: { $gte: min } } });
    }
    if (max) {
      max = parseInt(max);
      aQuery.push({ $match: { currentPrice: { $lte: max } } });
      aCountQuery.push({ $match: { currentPrice: { $lte: max } } });
    }
    // sắp xếp và thống kê
...
const getCourse = async (req, res, next) => {
  try {
      ...
      },
      {
        $project: {
          ...
          chapters: {
            ...
            number: 1,
            name: 1,
            lessons: { _id: 1, number: 1, title: 1, description: 1 },
          },
        },
      },
    ]);
    if (course[0]) {
      if (user) {
        const myCourse = await MyCourseModel.findOne({
          user,
          course: course[0],
        }).lean();
        if (myCourse) {
...
const getSuggestCourses = async (req, res, next) => {
  try {
    ...
    if (user) {
      ...
      const historySearchOfUser = await HistorySearchModel.findOne({
        user: user._id,
      }).lean();
      keyword = historySearchOfUser
        ? historySearchOfUser.historySearchs[0]
        : null;
      if (keyword) {
...
const deleteCourse = async (req, res, next) => {
  try {
    ...
    if (isBuyed) {
      return res
        .status(400)
        .json({ message: "Khoá học đã có người mua. Không thể xoá" });
    }
    ...
    let chapters = await ChapterModel.find({ course: course._id })
      .select("_id")
      .lean();
