...
const getAccountAndUsers = async (req, res, next) => {
  try {
    ...
    if (email) {
      ...
      aCountQuery.push({
        $match: { "account.email": new RegExp(email, "img") },
      });
    }
    if (role) {
    ...
    if (exports.toLowerCase().trim() == "true") {
      ...
      fs.createWriteStream(
        "./src/public/statistics/danh-sach-tai-khoan-nguoi-dung.xlsx"
      ).write(buffer);
...
const getStudents = async (req, res, next) => {
  try {
    ...
    if (exports.toLowerCase().trim() == "true") {
      ...
      var buffer = xlsx.build([{ name: "data", data: data }], { sheetOptions }); // Returns a buffer
      fs.createWriteStream(
        "./src/public/statistics/danh-sach-hoc-sinh.xlsx"
      ).write(buffer);
      return res.status(200).json({
        message: "ok",
        total,
...
const postMultiAccountAndUser = async (req, res, next) => {
  try {
        ...
        if (ValidateEmail(email)) {
          if (password != null && password.toString().trim() > 7) {
            ...
            fs.appendFileSync(
              ...
              `dòng ${
                i + 1
              }, lỗi password: '${password}' không hợp lệ hoặc bỏ trống \n`
            );
          }
        } else {
          fs.appendFileSync(
    ...
    if (sucess == data.length - 1) {
      res
        .status(201)
        .json({ message: `đã tạo ${sucess}/${data.length - 1} tài khoản` });
    } else {
      res.status(201).json({
    ...
    if (account !== null && typeof account === "object") {
      ...
      if (account.password) {
        if (account.password.trim().length > 7) {
          ...
          return res
            .status(400)
            .json({ message: "password phải dài hơn 7 ký tự" });
        }
      }
...
const deleteMultiAccountAndUser = async (req, res, next) => {
  try {
    ...
    if (logs == "") {
      return res
        .status(200)
        .json({ message: `update ${sucess}/${ids.length} oke` });
    }
    return res
      .status(200)
      .json({ message: `update ${sucess}/${ids.length} oke`, error: logs });
  } catch (error) {
