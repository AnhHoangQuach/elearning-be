...
const getMyCourse = async (req, res, next) => {
  try {
        ...
        item.chapters.forEach((chapter) => {
          ...
          chapter.lessons.map((lesson) => {
            ...
            for (let i = 0; i < item.progress.length; i++) {
              ...
              if (
                JSON.stringify(element.lessonId) == JSON.stringify(lesson._id)
              ) {
    ...
    try {
      ...
      const chapterOfLastView = await ChapterModel.findById(
        lastView.chapter
      ).lean();
...
const putProgress = async (req, res, next) => {
  try {
    ...
    if (lesson) {
      let isExisted =
        mc.progress?.some(
          (item) => JSON.stringify(item.lessonId) == JSON.stringify(lessonId)
        ) || false;
...
const postMyCourse = async (req, res, next) => {
  try {
    ...
    const isExisted = await MyCourseModel.findOne({
      user: req.user,
      course: course._id,
    }).lean();
