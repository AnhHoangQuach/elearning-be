...
courseApis.post(
  ...
  dontStorageUpload.single('image'),
  courseController.uploadImageToCloudinary
)
...
courseApis.get(
  '/:slug/related',
  passport.jwtAuthenticationOrNull,
...
courseApis.put(
  '/:slug',
  passport.jwtAuthentication,
  accessControl.grantAccess('updateOwn', 'course'),
...
courseApis.get(
  ...
  passport.jwtAuthentication,
  passport.isAdmin,
  courseController.getDetailPendingCourse
)

courseApis.post(
  '/upload/image',
  passport.jwtAuthentication,
  dontStorageUpload.fields([{ name: 'image' }]),
